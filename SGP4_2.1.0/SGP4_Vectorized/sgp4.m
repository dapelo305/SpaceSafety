...
%------------------------------- sgp4 ------------------------------
%-------------------------------------------------------------------
function [position, velocity] = sgp4(tsince, satdata)
...
if (isimp == 0)
    ...
    templ = templ + t3cof .* tcube + (t4cof + t5cof .*tsince) .* tfour;
end
a = aodp * (tempa.^2);
e = (satdata.eo) - tempe;
...
for i=1:3
	pos(i,:) = rk .* UV.v(i,:);
	vel(i,:) = rdotk .* UV.v(i,:) + rfdotk .* VV.v(i,:);
end
position = zeros(3,num);
velocity = zeros(3,num);
for i = 1:num
    [position(:,i), velocity(:,i)] = Convert_Sat_State(pos(:,i), vel(:,i));
end
